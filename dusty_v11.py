"""Dusty the vacuum, v9 -- a character rather than an appliance.
...
M_SUCK  = mat("dusty_suck", (0.06, 0.35, 0.45, 1), rough=0.15, emit=CYAN, emit_str=1.5)
# BLUE EYES. Ken, of v9: "Dusty's eyes are still white." They were cream balls
# with a cyan glow for a pupil, and at emit_str 2.0 the pupil blew out to white
# too, so the whole eye read as a ping-pong ball. The ball is now the blue --
# #9fe4ff, the blue Marty got, written in linear because that is what Blender's
# base colour is -- and the pupil is a deep navy that keeps the gaze readable
# against it, lit just enough not to go flat in a dim workshop.
EYE_BLUE = (0.35, 0.78, 1.00, 1)          # sRGB #9fe4ff
IRIS_NAVY = (0.02, 0.06, 0.26, 1)
M_EYE   = mat("dusty_eye", EYE_BLUE, rough=0.22)
# MARTY'S EYE, IN DUSTY'S COLOURS. Ken, of v10: "Somehow [Dusty's] eyes are a
# bit creepy. Maybe his eyes should be more like marty and ruby just a
# different color than black." Marty and Ruby share one eye -- a plain ball
# with a small pupil standing just proud of it, the pupil a bit under half the
# ball's radius, and no highlight at all. Dusty's had a pupil at 53% of the
# ball with a lit rim and a hard white spark on top, which is the recipe for a
# googly eye. The pupil comes down to Marty's proportion, the spark goes, and
# the pupil is INDIGO rather than the black those two wear -- his one point of
# difference, which is what Ken asked for.
IRIS_INDIGO = (0.03, 0.06, 0.30, 1)
M_IRIS  = mat("dusty_iris", IRIS_INDIGO, rough=0.28)
...
# DEEP INSIDE THE CAVITY, not filling it. v6 put the throat where it poked
# back out of the hole it had just cut, which read as a tongue -- or a boot.
# The dark sits well back, the glow further back still, so what you see is a
# mouth with something lit at the bottom of it.
ball("throat", 0.090, (0, -0.078, 0.146), M_DARK, parent=body, scale=(1.20, 0.80, 0.50))
# FAR ENOUGH FORWARD TO BE SEEN. Pushed to the back of the cavity it was
# honest and invisible; a pool of light just inside the lip is what tells you
# the mouth is working.
ball("gulp", 0.066, (0, -0.126, 0.146), M_SUCK, parent=body, scale=(1.02, 0.44, 0.40))
...
for s2, nm in ((-1, "L"), (1, "R")):
    ball("eye_" + nm, 0.055, (0.076 * s2, -0.168, 0.300), M_EYE, parent=body)
    # THE PUPIL FACES YOU. Flattened and set at the eyeball's own centre depth
    # it was half-buried, and he looked at the floor. A round pupil standing
    # just proud of the eye is what makes the gaze meet yours.
    ball("iris_" + nm, 0.024, (0.079 * s2, -0.212, 0.301), M_IRIS, parent=body)

torus("bumper", 0.168, 0.030, (0, 0, 0.030), M_DARK, parent=root)
# no chin: it was a cream lump under the mouth and read as something stuck to
# him. The bumper already gives the silhouette its base.
...
for s2, nm in ((-1, "L"), (1, "R")):
    # HELD FORWARD, WITH A CUFF. Straight out at his sides they were two balls
    # stuck on; brought round to the front, tilted down and given a dark cuff
    # where the sleeve ends, they read as mittens on a fellow ready to carry
    # something off.
    # AT HIS SIDES after all. Swung round to the front they fouled the mouth --
    # the thing the whole face is built around -- so they go back where they
    # were, smaller, with the dark cuff that makes a ball into a mitten.
    cyl("arm_" + nm, 0.018, 0.048, (0.186 * s2, -0.030, 0.172), M_DARK, bevel=0.007,
        parent=body, rot=(0, math.radians(74 * s2), 0))
    torus("cuff_" + nm, 0.024, 0.008, (0.203 * s2, -0.032, 0.166), M_DARK, parent=body,
          rot=(0, math.radians(74 * s2), 0))
    ball("mitt_" + nm, 0.038, (0.219 * s2, -0.033, 0.161), M_CREAM, parent=body,
         scale=(1.0, 0.94, 0.88))

# the bag rides on his back like a satchel, with a window so you can see he
# really is keeping what he took
# CREAM, NOT CORAL. The bag was the body's own colour on the body's own
# curve, so from three quarters behind it simply was not there. In cream it
# has a silhouette, and the strap and the little window say what it is.
ball("bag", 0.100, (0, 0.168, 0.238), M_CREAM, parent=body, scale=(1.0, 0.78, 0.90))
torus("bag_strap", 0.090, 0.013, (0, 0.150, 0.238), M_DARK, parent=body,
      rot=(math.radians(90), 0, math.radians(90)))
cyl("bag_window", 0.034, 0.018, (0, 0.246, 0.250), M_GLOW, parent=body,
