"""Dusty the vacuum, v5 -- a character rather than an appliance.
...
# A CHIN, not a bib. The old belly patch sat half inside the body and read as
# something sliding off him.
ball("chin", 0.080, (0, -0.176, 0.092), M_CREAM, parent=body, scale=(1.15, 0.30, 0.52))

# BIG EYES, PROUD OF THE FACE. Flattening them onto a panel (v3) made a
# bandit's mask; a cartoon's eyes stand out, and the off-centre white spark is
# what makes one look wet and alive rather than merely lit.
for s2, nm in ((-1, "L"), (1, "R")):
    ball("eye_" + nm, 0.062, (0.076 * s2, -0.150, 0.300), M_CREAM, parent=body)
    ball("iris_" + nm, 0.036, (0.080 * s2, -0.196, 0.298), M_GLOW, parent=body,
         scale=(1, 0.55, 1))
    ball("spark_" + nm, 0.014, (0.094 * s2, -0.208, 0.316), M_WHITE, parent=body,
         scale=(1, 0.6, 1))

# THE MOUTH IS THE INTAKE: a wide soft opening with the suction glowing inside
# it. v1 hung a funnel off his front and v3 a porthole; a mouth needs no
# explaining and does the same work.
# ...AND IT HAS TO COME THROUGH THE SURFACE. v4 put both of these INSIDE the
# sphere -- the body front sits at y -0.195 at this height, and an ellipsoid
# centred at -0.132 with a y-radius of 0.079 never reaches it. He came out a
# red ball with lovely eyes and no mouth at all. Placed by that arithmetic
# instead of by eye, the opening stands proud and reads from across a table.
ball("mouth", 0.110, (0, -0.190, 0.150), M_DARK, parent=body, scale=(1.15, 0.30, 0.55))
ball("gulp", 0.085, (0, -0.198, 0.150), M_GLOW, parent=body, scale=(1.10, 0.22, 0.45))
