...
#   2 a box holding a 1 (the pupil's)  3 a pad it reads out at the end
#
...
#   desk of its own, and everything next is the PUPIL's lesson: a fresh
#   number from the stack dropped on the 1 in its box (1 + 1 = 2), then Ruby
#   on the 1 in its thought, so it counts on any number; "Stop teaching it";
#   then the last pad, read out.
#
# Run, the teacher does all of that for real, and the pupil stands trained
# at its own desk with its box reading 2. Click the pupil and press Start:
# 3, 4, 5, ... The teacher itself runs once -- with the pupil gone from its
...
pupil = {'kind': 'robot', 'name': 'the pupil', 'program': [], 'condition': None,
         'trainedOn': None, 'team': [],
         'note': 'Untrained. The teacher gives it the box of 1 and shows it how to count.'}
counting = box(num(1))                                                 # noqa: F405
...
teacher = robot(                                                       # noqa: F405
    'the teacher',
    box(WILDTEXT, ANYROBOT, box(ANYNUM), WILDTEXT),                    # noqa: F405
    [take('given', 0), speak, put('given', 0),                         # noqa: F405
     take('given', 2),                                                 # noqa: F405
     teach(1),                                                         # the box goes to the pupil: its lesson begins
     taught(newnum),                                                   # noqa: F405
     taught(put('given', 0)),                                          # noqa: F405
     taught(erase(0)),                                                 # any number, not just 1
     {'type': 'endTeach'},
     take('given', 3), speak, put('given', 3)],                        # noqa: F405
    trained_on=work,
    note='A robot that trains a robot. It reads the first pad out, gives the '
         'box of 1 to the little robot in its second hole and teaches it to add '
         'a fresh 1 to what it was given, has Ruby loosen the 1 in the pupil’s '
         'thought so it counts on any number, and reads the last pad out. It runs '
         'once: with the pupil gone from its box, its thought no longer fits.')
...
RUN = ('TO RUN IT\n\n'
       ...
       'add a fresh 1 to its box,\n'
       'and have its thought\n'
       'loosened by Ruby.\n\n'
