...
"Tell my number" runs when a reading is on the nest, tells the number with
`[set | value | n]`, and moves the pad back. A pad crossing between two holes
is how two robots take turns here: there is no thought that says "an empty
nest". SPACE on the number starts them and it counts the milliseconds; "."
rests them. Generated by `make_timer.py`. The computer also answers
`[query | seconds | bird]`, the same clock in seconds.
