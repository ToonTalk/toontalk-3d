...
`[hours | minutes | seconds]`). Every letter needs a bird in its last hole to
answer to. The last page is `[set | value | _]`, the letter that tells a
number what it is. Generated by `make_computer.py`.

## timer.world.json

A timer built from one robot. Its pad is bound to a number on the table:
each round it takes the last reading off its nest, tells the number with
`[set | value | n]`, and asks the computer the time again to its own nest.
SPACE starts it and the number counts the milliseconds; "." rests it where
it is. Generated by `make_timer.py`.
