"""Dusty the vacuum, v3 -- a character rather than an appliance.
...
# A HEAD THAT MERGES INTO THE SHOULDERS. v2's first try put it on a neck and
# ringed it with a torus meant as a "groove" -- but a torus sits PROUD, so it
# read as a helmet ring, and the gap under it made him a hydrant. Set low
# enough to overlap the body, there is no gap and no ring.
head = ball("head", 0.145, (0, 0, 0.360), M_SHELL, parent=body, scale=(1.05, 1.0, 0.85))

# THE FACE IS A PANEL IN THE HEAD, like the robots' screen: a flattened dark
# lens that follows the curve of the front and barely stands out of it.
ball("visor", 0.115, (0, -0.090, 0.365), M_DARK, parent=head, scale=(1.0, 0.45, 0.72))
...
# FLATTENED, so they lie ON the panel rather than standing off it on stalks --
# the bulging version read as goggles. The off-centre white spark is what makes
# an eye look wet and alive rather than like a lamp.
for s, nm in ((-1, "L"), (1, "R")):
    ball("eye_" + nm, 0.040, (0.048 * s, -0.148, 0.372), M_CREAM, parent=head,
         scale=(1, 0.34, 1))
    ball("iris_" + nm, 0.024, (0.050 * s, -0.156, 0.370), M_GLOW, parent=head,
         scale=(1, 0.30, 1))
    ball("spark_" + nm, 0.009, (0.060 * s, -0.160, 0.382), M_WHITE, parent=head,
         scale=(1, 0.35, 1))

# antenna and blinker, the family badge Marty and the robots wear
cyl("antenna", 0.010, 0.075, (0, 0.015, 0.475), M_STEEL, bevel=0.004, parent=head)
ball("blinker", 0.026, (0, 0.015, 0.520), M_AMBER, parent=head)
...
# ON THE BODY, BELOW THE FACE. At head height it sat across his cheek like a
# gas mask; a muzzle belongs under the eyes, and small.
snout = cone("snout", 0.064, 0.050, 0.050, (0, -0.180, 0.238), M_CREAM, parent=body,
             rot=(math.radians(96), 0, 0))
torus("snout_lip", 0.058, 0.015, (0, -0.203, 0.235), M_DARK, parent=body,
      rot=(math.radians(96), 0, 0))
ball("smile", 0.044, (0, -0.199, 0.234), M_GLOW, parent=body, scale=(1, 0.30, 0.60))
empty("intake", (0, -0.30, 0.22), parent=body)     # where he points when working

# ---------------------------------------------------------------- arms
# He is the fellow who takes things away: give him something to take them with.
for s, nm in ((-1, "L"), (1, "R")):
    cyl("arm_" + nm, 0.024, 0.075, (0.180 * s, -0.02, 0.232), M_DARK, bevel=0.01,
        parent=body, rot=(0, math.radians(70 * s), 0))
    ball("mitt_" + nm, 0.044, (0.212 * s, -0.02, 0.208), M_CREAM, parent=body,
         scale=(1, 0.92, 0.86))

# ---------------------------------------------------------------- bag + wheels
# The bag rides high on his back like a satchel, with a cream patch so you can
# see it is a bag and not a hump.
bag = ball("bag", 0.100, (0, 0.155, 0.285), M_SHELL, parent=body, scale=(0.92, 0.78, 1.0))
torus("bag_strap", 0.088, 0.015, (0, 0.145, 0.285), M_CREAM, parent=body,
      rot=(math.radians(90), 0, math.radians(90)))
cyl("bag_window", 0.042, 0.018, (0, 0.222, 0.292), M_GLOW, parent=body,
